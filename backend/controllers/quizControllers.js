...
const getQuestions = expressAsyncHandler(async (req, res) => {
  ...
  try {
    const data = await Question.find(filter).select("desc options category language_id");
  ...
  for (let i = 0; i < pairs.length; i++) {
    ...
    try {
      ...
      if (question) {
        ...
        // Compare answer and givenAnswer as strings to avoid errors
        if (
          givenAns !== "-1" &&
          String(question.answer) === String(givenAns)
        ) {
          positiveMarks++;
        } else if (givenAns !== "-1" && String(question.answer) !== String(givenAns)) {
          negativeMarks++;
        } else {
          unAttempted++;
        }
